...
def gen_weight_path(seed_i, methods_i, da_method_i, pinzhong_i, ti, dax_i):
    ...
    info = str(seed_i)+methods_i+ da_method_i+pinzhong_i+ str(ti)+ '_dax'+str(dax_i)
    os.makedirs('D:/research3/Train_model/model/T12_DA/'+ methods_i +'/'+info +'/', exist_ok=True)
    weight_path = 'D:/research3/Train_model/model/T12_DA/'+ methods_i +'/'+info +'/'+ info
    return weight_path
...
epoch_num = 50; 
img_size = 64
seed_all = [1007,] #1004,1005,1006,1008,  1008,1009,1010,1011,1012,
train_all_data, test_all_data = load_data()
da_method = ['DA', ] # 'None','noise',  'rotate', 'Erase',
...
for seed_i in seed_all: # 遍历种子
    ...
    out_excel = ('./result/oneD_cnn_result_dbs/' +method_i+'_' +str(seed_i)+ '_' + str(t123) + '_DA.xlsx')
